...
pub fn run() {
    tauri::Builder::default()
        // Must be registered first: a second DVCC process exits here and the running instance
        // is brought to the front instead, so two processes never write the same data (F-3).
